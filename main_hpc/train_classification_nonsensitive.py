...
from arch import ClassifierLayerNoSens
...
#    device_ = 'cpu'
    ...
    class_net = ClassifierLayerNoSens([512,256,64])
    ...
    for epoch in range(10):
        ...
        for i, (inputs, labels, _) in enumerate(train_loader):
            ...
            loss = loss_crit(outputs.squeeze(), labels.squeeze())
            loss.backward()
            optimizer.step()
        ...
        for i, (inputs, labels, input_fair) in enumerate(valid_loader):
            inputs, labels, _ = inputs.to(device_), labels.to(device_), input_fair.to(device_)
